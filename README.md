...
| 🗂️ **历史档案管理器 (GUI)** | 离线 Tkinter 桌面程序 · 打开已保存的 SQLite 数据库即可查看/筛选/统计 · 不依赖网络/浏览器 💕
| 🏷️ **迈瑞 MDC 报警解析引擎** | 识别 `MDC_EVT_AFIB`、`MDC_EVT_VFIB`、`CARD_BEAT_RATE_IRREG` 等 50+ 代码并翻译为"房颤/室颤/心率不齐"，与生命体征自动关联显示为"监护仪检测"列
| 📈 **统计分析与趋势图** | 档案管理器内置：HR/SpO₂/BP/RR 趋势折线图 · 按科室/日期筛选 · 入院诊断与监护仪诊断分开显示
...
├── start_archive.py             # 🗂️ 历史档案管理器（离线 Tkinter GUI · 双击即可打开数据库）
│
├── app/
...
│   │
│   ├── archive/                 # 🗂️ 历史档案管理器（离线桌面端 · 无需浏览器/网络）
│   │   ├── history_manager.py   #    数据库查询 + MDC 报警解析引擎 + 统计分析
│   │   └── gui.py               #    Tkinter GUI 主窗口（表格 + 详情 + 趋势图）
│   │
│   └── mindray_alarm/           # 🏷️ 迈瑞 MDC 报警代码映射表（可离线查阅 · 50+ 项）
│       ├── codes.py             #    MDC_EVT_XXX / CARD_XXX 等代码 → 中文诊断翻译
│       ├── parser.py            #    报警文本解析器（正则 + 关键词匹配）
│       └── example.py           #    使用示例（独立脚本演示如何解析报警）
│
...
### 方式三：历史档案管理器（离线数据分析 · GUI 桌面程序）

如果只是想回看/分析已经保存下来的数据库，完全不需要启动 Web 服务，只要打开 GUI 就行：

```bash
pip install -r requirements.txt   # （已经装过就跳过这步）
python start_archive.py
```

启动后你会看到一个粉紫色 Tkinter 桌面窗口：
- 左上角「选择数据库」→ 挑一个你之前跑过的 `.db` 文件
- 中间大表格：每条生命体征记录 + **监护仪检测**列（由 MDC 报警代码自动翻译的真实诊断）
- 双击任意一行：查看患者详情 + **入院诊断**（录入的）与 **监护仪检测**（实时的）两栏分开
- 「趋势图」按钮：按患者或按全体绘制 HR/SpO₂/BP/RR 曲线
- 「统计」按钮：按科室 / 日期 / 诊断统计分布

> 💡 历史档案管理器完全离线运行，不会启动任何网络端口，GUI 自带 matplotlib 画图。如果你只在值班电脑上回看，选这个就够了 🖤
...
| `python start_archive.py` | **历史档案管理器**（Tkinter 桌面 GUI） | 回头看历史数据 / 统计分析 / 打印病例 · 离线用 · 无需浏览器 💕 |
...
> 💡 **很重要的一点**：本小姐保留了所有报警的**原始文本**！同时 `app/mindray_alarm/mdc_codes.py` 内置了一份迈瑞 MDC 报警代码映射表，能把 `MDC_EVT_PVC_FREQ`、`MDC_EVT_BP_SYS_HIGH`、`CARD_BEAT_RATE_IRREG` 等 50+ 种代码翻译成"室早频发/收缩压过高/心率不齐"等中文诊断，并在历史档案管理器的"监护仪检测"列里自动显示。如果是未识别的新代码，前端会直接把报警原文贴出来，方便你查迈瑞操作手册。毕竟医院里那些莫名其妙的报警代码真的超讨厌对吧 🖤
...
pydantic
python-dotenv
matplotlib                     # 历史档案管理器 · 趋势折线图
tkintertable                   # 历史档案管理器 · Tkinter 表格（可选，缺省时自动降级为原生 Treeview）
```
...
**不需要专用交换机，不需要额外的硬件，不需要从监护仪拉网线到专用中央站主机，也不需要花钱买任何 License。就算监护仪插在你科室原有的路由器/交换机上就行，只要能 ping 通 2575 端口，就能抓到数据 🖤**
